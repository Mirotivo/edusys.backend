...
public class SubscriptionService : ISubscriptionService
{
    ...
    public async Task<(int SubscriptionId, int TransactionId)> CreateSubscriptionAsync(SubscriptionRequestDto request, string userId)
    {
        ...
        {
            basePrice = request.PaymentType == PaymentType.StudentMembership ? studentMonthlyPrice : tutorMonthlyPrice;
            subscriptionMonths = 1;
        }
        else // Annual Plan (12 months paid, 3 extra months free)
        {
            basePrice = request.PaymentType == PaymentType.StudentMembership ? studentAnnualPrice : tutorAnnualPrice;
    ...
    public async Task<List<Subscription>> ListUserSubscriptionsAsync(string userId)
    {
        return await _dbContext.Subscriptions
    ...
    /// </summary>
    public async Task<bool> ChangeBillingFrequencyAsync(string userId, BillingFrequency newFrequency)
    {
        var subscription = await _dbContext.Subscriptions
        ...
        subscription.NextBillingDate = DateTime.UtcNow.AddMonths(newFrequency == BillingFrequency.Monthly ? 1 : 12);
    ...
    private string GetActionDescription(SubscriptionHistory history)
    {
        ...
        if (history.BillingFrequency == BillingFrequency.Monthly)
            return "Switched to Monthly Plan";

        if (history.BillingFrequency == BillingFrequency.Yearly)
