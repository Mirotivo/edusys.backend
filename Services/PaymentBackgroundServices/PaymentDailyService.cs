            ...
            using (var scope = _serviceProvider.CreateScope())
            {
                ...
                foreach (var subscription in subscriptionsToRenew)
                {
                    try
                    {
                        ...
                        var paymentMethod = user.UserCards.FirstOrDefault(c => c.Type == CardType.Paying);
                        ...
                        var request = new SubscriptionRequestDto
                        {
                            ...
                            PaymentType = PaymentType.StudentMembership, // Adjust based on actual type
                            Amount = subscription.Amount
                        };

                        // Reuse CreateSubscription method to handle renewal
                        var (subscriptionId, transactionId) = await subscriptionService.CreateSubscription(request, user.Id);
