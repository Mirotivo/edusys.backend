...
public class UserService : IUserService
{
    ...
    public async Task<List<object>> GetLandingPageUsersAsync()
    {
        ...
        var predefinedLocations = new List<object>
        {
            new { Img = "assets/img/city/city_sydney.jpg", City = "Sydney", Country = "Australia", Mentors = jobLocations.GetValueOrDefault("Sydney", 0) },
            new { Img = "assets/img/city/city_brisbane.jpg", City = "Brisbane", Country = "Australia", Mentors = jobLocations.GetValueOrDefault("Brisbane", 0) },
            new { Img = "assets/img/city/city_perth.jpg", City = "Perth", Country = "Australia", Mentors = jobLocations.GetValueOrDefault("Perth", 0) }
        };
    ...
    public async Task<bool> ModifyPaymentScheduleAsync(string userId, PaymentSchedule paymentSchedule)
    {
        var user = await _userManager.FindByIdAsync(userId);
        if (user == null)
        {
    ...
    private async Task HandleReferralTokenAsync(string referralToken, string newUserId)
    {
        ...
        {
            BillingFrequency = BillingFrequency.Monthly.ToString(),
